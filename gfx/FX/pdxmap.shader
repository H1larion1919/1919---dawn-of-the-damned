...
{	
	...
	[[		
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			// Iron Curtain terrain remains visible at close and medium camera ranges,
			// then fades into the flat political colour buffer at strategic zoom.
			float vFlatPoliticalZoom = terrain_flatten_zoom_out( gradient_border_camera_distance() );
			vOut = lerp( vOut, diffuse.rgb, vFlatPoliticalZoom );
			vOut = apply_screen_red_tint( vOut );
