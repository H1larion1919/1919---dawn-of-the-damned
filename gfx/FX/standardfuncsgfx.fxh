...
PixelShader = 
{
	...
	float terrain_flatten_zoom_out( float vCamDist )
	{
		float vZoomOut = saturate( ( 1.0f - vCamDist ) / GB_CAM_MAX_FILLING_CLAMP );
		return smoothstep( TERRAIN_FLATTEN_START, TERRAIN_FLATTEN_END, vZoomOut );
	}

	float3 apply_water_depth_overlay( float3 vWaterColor, float vWaterHeight )
	{
		float vDepth = 1.0f - smoothstep( WATER_DEPTH_DEEP_HEIGHT, WATER_DEPTH_SHALLOW_HEIGHT, vWaterHeight );
		float vDepthGradient = lerp( WATER_DEPTH_GRADIENT_SHALLOW, WATER_DEPTH_GRADIENT_DEEP, vDepth );

		// The depth gradient is the base and water colour is the Overlay layer.
		// Both configured gradient limits are below 0.5, so this is the exact
		// dark-half Overlay equation: 2 * base * blend.
		float3 vDepthOverlay = saturate( 2.0f * vDepthGradient * vWaterColor );
		return lerp( vWaterColor, vDepthOverlay, WATER_DEPTH_OVERLAY_STRENGTH );
	}

	float3 apply_screen_red_tint( float3 vColor )
	{
		float3 vWarmColor = saturate( vColor * SCREEN_RED_TINT_MULTIPLY + SCREEN_RED_TINT_ADD );
		return lerp( vColor, vWarmColor, SCREEN_RED_TINT_STRENGTH );
	}
